...
def ensure_state_defaults(st: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # Backward compat: migrate spent_usd → spent_rub (prices were 0, so reset to 0)
    if "spent_usd" in st and "spent_rub" not in st:
        st["spent_rub"] = 0.0
        del st["spent_usd"]
    st.setdefault("spent_rub", 0.0)
...
def init_state() -> Dict[str, Any]:
    ...
    try:
        ...
        st["session_spent_snapshot"] = float(st.get("spent_rub") or 0.0)

        ground_truth = check_provider_ground_truth()
        if ground_truth is not None:
            st["session_total_snapshot"] = ground_truth["total_rub"]
            st["cloudru_total_rub"] = ground_truth["total_rub"]
            st["cloudru_daily_rub"] = ground_truth["daily_rub"]
            st["cloudru_last_check_at"] = datetime.datetime.now(datetime.timezone.utc).isoformat()
        else:
...
EVOLUTION_BUDGET_RESERVE: float = 200.0  # Stop evolution when remaining < this (RUB)
...
def budget_remaining(st: Dict[str, Any]) -> float:
    """Calculate remaining budget in RUB."""
    spent = float(st.get("spent_rub") or 0.0)
...
def check_provider_ground_truth() -> Optional[Dict[str, float]]:
    """Stub: Cloud.ru does not expose a usage/ground-truth API. Returns None."""
    return None


# Backward-compatible aliases
check_openrouter_ground_truth = check_provider_ground_truth
check_cloudru_ground_truth = check_provider_ground_truth
...
def update_budget_from_usage(usage: Dict[str, Any]) -> None:
    ...
    Every 50 calls, attempts ground truth check for comparison.
    """
    ...
    try:
        ...
        if cost is None:
            cost = 0.0
        st["spent_rub"] = _to_float(st.get("spent_rub") or 0.0) + _to_float(cost)
    ...
    # Step 2: ground truth check OUTSIDE the lock
        ...
        if ground_truth is not None:
            ...
            try:
                ...
                if session_total_snap is not None and session_spent_snap is not None:
                    current_total_rub = ground_truth["total_rub"]
                    current_spent_rub = _to_float(st.get("spent_rub") or 0.0)
                    or_delta = current_total_rub - _to_float(session_total_snap)
                    our_delta = current_spent_rub - _to_float(session_spent_snap)
...
def budget_breakdown(st: Dict[str, Any]) -> Dict[str, float]:
    ...
    Reads llm_usage events and aggregates cost by category field.
    Returns dict like {"task": 12.5, "evolution": 45.2, ...} (values in RUB)
    """
...
def model_breakdown(st: Dict[str, Any]) -> Dict[str, Dict[str, float]]:
    ...
    Returns dict like:
    {
        "ai-sage/GigaChat3-10B-A1.8B": {"cost": 1.5, "calls": 120, "prompt_tokens": 50000, "completion_tokens": 3000},
    }
    """
    events_path = DRIVE_ROOT / "logs" / "events.jsonl"
    if not events_path.exists():
...
def status_text(workers_dict: Dict[int, Any], pending_list: list, running_dict: Dict[str, Dict[str, Any]],
    ...
    spent = float(st.get("spent_rub") or 0.0)
    pct = budget_pct(st)
    remaining = max(0, TOTAL_BUDGET_LIMIT - spent)
    lines.append(f"budget_total: {TOTAL_BUDGET_LIMIT:.0f} ₽")
    lines.append(f"budget_remaining: {remaining:.0f} ₽")
    if pct > 0:
        lines.append(f"spent_rub: {spent:.2f} ₽ ({pct:.1f}% of budget)")
    else:
        lines.append(f"spent_rub: {spent:.2f} ₽")
    ...
    if drift_pct is not None:
        session_total_snap = st.get("session_total_snapshot")
        session_spent_snap = st.get("session_spent_snapshot")
        provider_total = st.get("cloudru_total_rub") or st.get("cloudru_total_usd") or st.get("openrouter_total_usd")

        if session_total_snap is not None and session_spent_snap is not None and provider_total is not None:
            or_delta = provider_total - session_total_snap
