...
def _build_runtime_section(env: Any, task: Dict[str, Any]) -> str:
    ...
    try:
        ...
        spent_rub = float(state_data.get("spent_rub") or state_data.get("spent_usd") or 0)
        total_rub = float(os.environ.get("TOTAL_BUDGET", "1000"))
        remaining_rub = total_rub - spent_rub
        budget_info = {"total_rub": total_rub, "spent_rub": spent_rub, "remaining_rub": remaining_rub}
    except Exception:
...
def _build_health_invariants(env: Any) -> str:
    ...
    try:
        ...
        if state_data.get("budget_drift_alert"):
            ...
            our = state_data.get("spent_rub") or state_data.get("spent_usd") or 0
            theirs = state_data.get("cloudru_total_rub") or state_data.get("cloudru_total_usd") or state_data.get("openrouter_total_usd") or 0
            checks.append(f"WARNING: BUDGET DRIFT {drift_pct:.1f}% — tracked={float(our):.2f} ₽ vs provider={float(theirs):.2f} ₽")
        else:
    ...
    try:
        ...
        costly = [t for t in per_task_cost_summary(5) if t["cost"] > 500.0]
        for t in costly:
            checks.append(
        ...
        if not costly:
            checks.append("OK: no high-cost tasks (>500 ₽)")
    except Exception:
        pass
...
def apply_message_token_soft_cap(
    ...
    for prefix in prunable:
        ...
        for i, msg in enumerate(pruned):
            ...
            # Handle multipart content (trim from the dynamic text block)
            if isinstance(content, list) and msg.get("role") == "system":
                # Dynamic block is the last text block in the multipart system content.
                dynamic_block = None
