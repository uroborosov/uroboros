...
# Static pricing for Cloud.ru Foundation Models (RUB/1M tokens: input, cached, output).
# Cloud.ru pricing: https://cloud.ru/docs/foundation-models/ug/topics/pricing
# GLM-4.7 pricing not published yet — set to 0; update when available.
_MODEL_PRICING_STATIC = {
    "zai-org/GLM-4.7": (0.0, 0.0, 0.0),
    "ai-sage/GigaChat3-10B-A1.8B": (0.0, 0.0, 0.0),
}
...
def _get_pricing() -> Dict[str, Tuple[float, float, float]]:
    """Return static pricing table. Cloud.ru does not expose a public pricing API."""
...
def _estimate_cost(model: str, prompt_tokens: int, completion_tokens: int,
    ...
    """Estimate cost in RUB from token counts using known pricing. Returns 0 if model unknown."""
...
def _check_budget_limits(
    ...
    if budget_pct > 0.5:
        finish_reason = f"Task spent {task_cost:.2f} ₽ (>50% of remaining {budget_remaining:.2f} ₽). Budget exhausted."
        ...
        messages.append({"role": "user", "content": f"[INFO] Task spent {task_cost:.2f} ₽ of {budget_remaining:.2f} ₽. Wrap up if possible."})
...
def run_llm_loop(
    ...
    Args:
        budget_remaining: If set (RUB), forces completion when task cost exceeds 50% of this budget
    ...
    try:
        while True:
            ...
            budget_result = _check_budget_limits(
                budget_remaining, accumulated_usage, round_idx, messages,
...
def _call_llm_with_retry(
    ...
    for attempt in range(max_retries):
        try:
            ...
            tool_calls = msg.get("tool_calls") or []
            content = msg.get("content")
            # GLM-4.7 may put its answer in reasoning_content; llm.py promotes it,
