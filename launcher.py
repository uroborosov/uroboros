...
def _migrate_old_settings() -> None:
    ...
    # Backward compat: migrate legacy OPENROUTER_API_KEY env → API_KEY
    legacy_key = os.environ.get("OPENROUTER_API_KEY", "")
    if legacy_key and "API_KEY" not in migrated:
        migrated["API_KEY"] = legacy_key
    ...
    if old_settings.exists():
        try:
            ...
            # Backward compat: migrate legacy OPENROUTER_API_KEY from old settings
            if old.get("OPENROUTER_API_KEY") and "API_KEY" not in migrated:
                migrated["API_KEY"] = old["OPENROUTER_API_KEY"]
        except Exception:
            pass
...
<div class="card">
  ...
  <h3>Cloud LLM (Cloud.ru Foundation Models)</h3>
  <label>API Key <span class="opt">— required for cloud models</span></label>
  <input id="api-key" type="password" placeholder="Key Secret from Cloud.ru console" autofocus>
  <p class="hint">Generate at <a href="https://cloud.ru" target="_blank">cloud.ru</a> → Service Accounts → API Keys</p>
  <div class="row">
    <div class="field"><label>Main Model</label><input id="model" value="ai-sage/GigaChat3-10B-A1.8B"></div>
    <div class="field"><label>Budget (₽)</label><input id="budget" type="number" value="1000" min="1" step="1" style="width:100px"></div>
  </div>
...
btn.addEventListener('click', async () => {
    ...
    const data = {
        TOTAL_BUDGET: parseFloat(document.getElementById('budget').value) || 1000,
        OUROBOROS_MODEL: document.getElementById('model').value.trim() || 'zai-org/GLM-4.7',
    };
    const orKey = keyInput.value.trim();
    if (orKey.length >= 10) data.API_KEY = orKey;
...
def _run_first_run_wizard() -> bool:
    ...
    settings = _load_settings()
    if settings.get("API_KEY") or settings.get("LOCAL_MODEL_SOURCE"):
        return True
    ...
    class WizardApi:
        def save_wizard(self, data: dict) -> str:
            ...
            if len(key) < 10 and not has_local:
                return "Provide a Cloud.ru API key or select a local model."
