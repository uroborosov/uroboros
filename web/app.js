...
function initDashboard() {
    ...
    async function updateDashboard() {
        try {
            ...
            const spent = data.spent_rub || data.spent_usd || 0;
            const limit = data.budget_limit || 1000;
...
function initSettings() {
    ...
    page.innerHTML = `
        ...
        <div class="settings-scroll">
            <div class="form-section">
                <h3>API Keys</h3>
                <div class="form-row"><div class="form-field"><label>Cloud.ru API Key</label><input id="s-apikey" type="password" placeholder="Key Secret from Cloud.ru"></div></div>
            ...
            <div class="divider"></div>
                ...
                <div class="form-row" style="align-items:flex-end">
                    <div class="form-field"><label>Main Model</label><select id="s-model" style="width:250px"><option value="zai-org/GLM-4.7">zai-org/GLM-4.7</option></select></div>
                    <label class="local-toggle"><input type="checkbox" id="s-local-main" disabled> Local</label>
                </div>
                <div class="form-row" style="align-items:flex-end">
                    <div class="form-field"><label>Code Model</label><select id="s-model-code" style="width:250px"><option value="zai-org/GLM-4.7">zai-org/GLM-4.7</option></select></div>
                    <label class="local-toggle"><input type="checkbox" id="s-local-code" disabled> Local</label>
                </div>
                <div class="form-row" style="align-items:flex-end">
                    <div class="form-field"><label>Light Model</label><select id="s-model-light" style="width:250px"><option value="zai-org/GLM-4.7">zai-org/GLM-4.7</option></select></div>
                    <label class="local-toggle"><input type="checkbox" id="s-local-light" disabled> Local</label>
                </div>
                <div class="form-row" style="align-items:flex-end">
                    <div class="form-field"><label>Fallback Model</label><select id="s-model-fallback" style="width:250px"><option value="zai-org/GLM-4.7">zai-org/GLM-4.7</option></select></div>
            ...
            <div class="form-section">
                ...
                <div class="form-row">
                    ...
                    <div class="form-field"><label>Total Budget (₽)</label><input id="s-budget" type="number" min="1" value="1000" style="width:120px"></div>
                </div>
                <div class="form-row">
    ...
    // Populate a <select> with model options, preserving current value
    function populateModelSelect(selectId, models, currentValue) {
        const sel = document.getElementById(selectId);
        const val = currentValue || sel.value;
        sel.innerHTML = '';
        const seen = new Set();
        // Ensure the saved value is always an option (even if not in API list)
        if (val && !models.includes(val)) models = [val, ...models];
    ...
    // Load models list + settings in parallel, then apply
    const modelSelectIds = ['s-model', 's-model-code', 's-model-light', 's-model-fallback'];
    Promise.all([
        fetch('/api/models').then(r => r.json()).catch(() => ({ models: ['zai-org/GLM-4.7'] })),
        fetch('/api/settings').then(r => r.json()).catch(() => ({})),
    ]).then(([modelsResp, s]) => {
        const models = modelsResp.models || ['zai-org/GLM-4.7'];
        const savedValues = {
            's-model': s.OUROBOROS_MODEL,
            's-model-code': s.OUROBOROS_MODEL_CODE,
            's-model-light': s.OUROBOROS_MODEL_LIGHT,
            's-model-fallback': s.OUROBOROS_MODEL_FALLBACK,
        };
        for (const id of modelSelectIds) {
            populateModelSelect(id, [...models], savedValues[id] || '');
        }

        if (s.API_KEY) document.getElementById('s-apikey').value = s.API_KEY;
    ...
    document.getElementById('btn-save-settings').addEventListener('click', async () => {
        const body = {
            OUROBOROS_MODEL: document.getElementById('s-model').value || 'zai-org/GLM-4.7',
            OUROBOROS_MODEL_CODE: document.getElementById('s-model-code').value || 'zai-org/GLM-4.7',
            OUROBOROS_MODEL_LIGHT: document.getElementById('s-model-light').value || 'zai-org/GLM-4.7',
            OUROBOROS_MODEL_FALLBACK: document.getElementById('s-model-fallback').value || 'zai-org/GLM-4.7',
            ...
            TOTAL_BUDGET: parseFloat(document.getElementById('s-budget').value) || 1000,
        ...
        const orKey = document.getElementById('s-apikey').value;
        if (orKey && !orKey.includes('...')) body.API_KEY = orKey;
...
function initLogs() {
    ...
    function buildLogMessage(evt) {
        ...
        if (t === 'llm_round' || t === 'llm_usage') {
            ...
            if (evt.cost_rub) parts.push(`${Number(evt.cost_rub).toFixed(4)} ₽`);
            else if (evt.cost_usd) parts.push(`${Number(evt.cost_usd).toFixed(4)} ₽`);
