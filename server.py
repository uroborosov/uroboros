...
def _run_supervisor(settings: dict) -> None:
    ...
    try:
        ...
        # ── PATCH: self-destruct protection (2026-03-08) ──────────
        #
        # BUG: при systemctl restart без настроенного GitHub remote
        #   ensure_repo_present() создавал пустой git-репо (только .gitignore),
        #   затем safe_restart() делал reset --hard к этому пустому коммиту
        #   → весь код в REPO_DIR удалялся безвозвратно.
        #
        # БЫЛО (оригинал):
        #   ensure_repo_present()
        ...
        #   ok, msg = safe_restart(reason="bootstrap", ...)
        #
        # СТАЛО: проверяем — если код на месте, но remote не настроен,
        #   пропускаем деструктивный bootstrap. Если remote есть —
        #   всё работает как раньше (pull + reset к актуальному коду).
        #
        _code_intact = (REPO_DIR / "server.py").exists() or (REPO_DIR / "ouroboros").is_dir()
        _repo_slug = settings.get("GITHUB_REPO", "")
        _gh_token = settings.get("GITHUB_TOKEN", "")
        _has_remote = bool(_repo_slug and _gh_token)

        if _code_intact and not _has_remote:
            log.warning(
...
# ---------------------------------------------------------------------------
# Models list endpoint (Cloud.ru /v1/models proxy with cache)
# ---------------------------------------------------------------------------
